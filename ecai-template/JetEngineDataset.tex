...
and $\Delta SE$ are approximately $-1\%$ and $-5\%$, respectively, thereby simulating tip clearance faults of varying severity.
...
and $\Delta SE$ typically range from approximately $+3\%$ to $-1\%$, respectively, resulting in a ratio of $-3$.
...
The combined effect in this scenario is additive, with deviations approximately around $+1\%$ and $-5\%$,
...
various health conditions may develop, as illustrated in Figure~\ref{fig:health_conditions}.
...
\begin{figure}[h]
  ...
  \label{fig:health_conditions}
\end{figure}

\subsection{Dataset Characteristics}

The dataset is available in compressed CSV format (csv.gz) at ... Figure~\ref{fig:csv} shows the first lines
...
his scenario exemplifies a specific fault condition characterized by a severe VGV fault leading to FOD.
The damage is attributed to the detachment of a particle from the VGV mechanism, such as a loosened bolt, which is
subsequently ingested by the engine. The failure of the VGV mechanism may be preceded by a transient VGV
fault event, serving as an early indicator of the impending failure.
